...
function getHistory(){
	...
	$sql = "select * from chat where (chat.to = '".mysql_real_escape_string($_SESSION['username'])."' OR chat.to = '".mysql_real_escape_string($_GET['chatuser'])."') AND (chat.from = '".mysql_real_escape_string($_SESSION['username'])."' OR chat.from = '".mysql_real_escape_string($_GET['chatuser'])."') order by id ASC";
	$query = mysql_query($sql);
